...
class HashTable:
    ...
    def insert(self, strr):
        ...
        self.T[insertKey].insert(strr, self.T[insertKey].head)
...
def suggestions(htable, word):
    words = []
    for i in range(len(word)):
        for j in "abcdefghijklmnopqrstuvwxyz":
            rep = word[:i] + j + word[i + 1:]
            if htable.search(rep):
                words.append(rep)
    return words
...
def main():
    ...
    for i in range(30):
        print(dictionary.T[dictionary.keys()[i]].print())
        print()
        print()
    ...
    if dictionary.search(w):
        ...
        print(suggestions(dictionary, w))
